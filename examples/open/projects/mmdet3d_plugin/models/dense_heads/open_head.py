...
class OpenHead(StreamPETRHead):
    ...
    def forward(self, outs_roi, img_metas, topk_indexes=None, **data):
        ...
        outs_dec, all_cls_scores, all_bbox_preds = self.transformer(
            memory,
            tgt,
            query_pos,
            pos_embed,
    ...
    def position_embeding(self, data, img_feats, img_metas, outs_roi):
        ...
        centers2d = (
            reference_points[..., 0:2].reshape([B, N, H, W, 2]).permute(0, 1, 3, 2, 4)
        )
        centers_depth = (
            reference_points[..., 2:3].reshape([B, N, H, W, 1]).permute(0, 1, 3, 2, 4)
        )
        centers2d[..., 0] *= pad_w
        centers2d[..., 1] *= pad_h
        centers_depth = (
            centers_depth * (self.max_depth - self.min_depth) + self.min_depth
        )
    ...
    def gen_dfl_score(self, bbox_preds, bbox_targets):
        bboxes3d = denormalize_bbox(
            bbox_preds.reshape(-1, bbox_preds.size(-1)), self.pc_range
        )
    ...
    def temporal_alignment(self, query_pos, tgt, reference_points):
        ...
        temp_pos = self.query_embedding(
            pos2posemb3d(inverse_sigmoid(temp_reference_point))
        )
