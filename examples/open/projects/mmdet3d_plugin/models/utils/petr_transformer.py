...
class PETRTransformerDecoder(TransformerLayerSequence):
    ...
    def __init__(
        ...
        self._trim, self._tgtg_info = build_gbc(
            r=kwargs.pop("r", 0),
            n=kwargs.pop("n", 0),
            k=kwargs.pop("k", 0),
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        _tgtg_enable = self._tgtg_info["enable"]
        key_padding_mask = None
        ...
        for lvl, layer in enumerate(self.layers):
            ...
            if self.return_intermediate:
                if (not _tgtg_enable) or (_tgtg_enable and lvl < self._tgtg_info["n"]):
                    if self.post_norm is not None:
                        intermediate.append(torch.nan_to_num(self.post_norm(query)))
                    else:
                        intermediate.append(torch.nan_to_num(query))
                    cls_scores = cls_branches[lvl](intermediate[-1])
                    bbox_preds = reg_branches[lvl](intermediate[-1])
                    bbox_preds[..., 0:3] += reference[..., 0:3]
                    bbox_preds[..., 0:3] = bbox_preds[..., 0:3].sigmoid()
                    outputs_class.append(cls_scores)
                    outputs_coord.append(bbox_preds)

            # [Nk, bs, E] -> [bs, Nk, E]
            key = key.contiguous().transpose(0, 1)
            cls_scores = (
                outputs_class[-1].contiguous().transpose(0, 1)
                if len(outputs_class) > 0
                else None
            )
            _trim = self._trim(
                cls_scores=cls_scores,
                attn_weights=attn if (attn is not None) else None,  # [Nq, Nk]
                metric=key,  # [Nk, E]
                r=self._tgtg_info["r"][lvl],
                k=self._tgtg_info["k"],
            )

            # [bs, Nk-r, E] -> [Nk-r, bs, E]
            key = value = _trim(key).contiguous().transpose(0, 1)
            key_pos = (
                _trim(key_pos.contiguous().transpose(0, 1)).contiguous().transpose(0, 1)
            )

        if not self.return_intermediate or _tgtg_enable:
            if self.post_norm is not None:
                query = torch.nan_to_num(self.post_norm(query))
            else:
                query = torch.nan_to_num(query)
            cls_scores = cls_branches[-1](query)
            bbox_preds = reg_branches[-1](query)
            bbox_preds[..., 0:3] += reference[..., 0:3]
            bbox_preds[..., 0:3] = bbox_preds[..., 0:3].sigmoid()
            return query[None], cls_scores[None], bbox_preds[None]

        return (
            torch.stack(intermediate),
            torch.stack(outputs_class),
            torch.stack(outputs_coord),
        )
...
class PETRTemporalTransformer(BaseModule):
    ...
    def forward(
        ...
        cls_branches: ModuleList = None,
        reg_branches: ModuleList = None,
    ):
        ...
        out_dec, outputs_class, outputs_coord = self.decoder(
            query=tgt,
            key=memory,
            value=memory,
            key_pos=pos_embed,
            ...
            attn_masks=(
                attn_masks if isinstance(attn_masks, list) else [attn_masks, None]
            ),
        ...
        outputs_class = outputs_class.transpose(1, 2).contiguous()
        outputs_coord = outputs_coord.transpose(1, 2).contiguous()
        return out_dec, outputs_class, outputs_coord
